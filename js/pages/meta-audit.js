/**
 * 数据中台 V4.0 - 数据治理 / 元数据审核
 */
window.DP = window.DP || {};
DP.pages = DP.pages || {};

DP.pages.metaAudit = (function () {
  var rows = {
    pending: [
      {
  ...
  function getChangeTypeClass(changeType) {
    if (changeType === '新增') return 'ma-change-add';
    if (changeType === '删除') return 'ma-change-delete';
  ...
  function renderToolbar(type) {
    ...
    '<div class="ma-toolbar-right">' +
      '<div class="ma-filter"><span>类型:</span><select class="ma-select ma-select-sm"><option>请选择</option><option>新增</option><option>修改</option><option>删除</option></select></div>' +
    ...
    return [
      ...
      },
      {
        ...
        items: [
          ...
          { name: '版本', before: 'V1.0', after: 'V1.1', changed: true },
      ...
      },
      {
        title: '其它',
        items: [
          { name: '备注', before: '沿用原字段说明。', after: row.reason, changed: true }
        ]
      }
    ];
  }
  ...
  function renderMetaCurrentRows(row) {
    var groups = getMetaGroups(row);
    var valueKey = row.changeType === '删除' ? 'before' : 'after';
    return groups.map(function (group) {
      var rowsHtml = group.items.map(function (item) {
        return '<tr>' +
  ...
  function renderAuditRecords(row, mode) {
    if (mode === 'detail') {
      ...
      return '<div class="ma-record-list">' +
        '<div class="ma-record-item"><div class="ma-record-dot"></div><div class="ma-record-main"><div class="ma-record-top"><span class="ma-record-state ' + recordStateClass + '">' + row.status + '</span><b>王敏</b><span>' + row.finishTime + '</span></div><div class="ma-record-desc">处理说明：' + (row.status === '审核通过' ? '审核通过，元数据内容符合治理规范。' : (row.status === '审核驳回' ? '审核驳回，请补充依据后重新提交。' : '等待当前审核人处理。')) + '</div></div></div>' +
        '<div class="ma-record-item"><div class="ma-record-dot"></div><div class="ma-record-main"><div class="ma-record-top"><span class="ma-record-state success">审核通过</span><b>李娜</b><span>2026-05-10 10:05:00</span></div><div class="ma-record-desc">处理说明：一级审核通过，字段口径调整合理。</div></div></div>' +
  ...
  function renderHandlePage(row, mode) {
    ...
    var isModify = row.changeType === '修改';
    var auditContentTable = isModify
      ? '<table class="ma-diff-table"><thead><tr><th>元数据属性</th><th>修改前</th><th>修改后</th></tr></thead><tbody>' + renderMetaChangeRows(row) + '</tbody></table>'
      : '<table class="ma-diff-table ma-current-table"><thead><tr><th>元数据属性</th><th>当前内容</th></tr></thead><tbody>' + renderMetaCurrentRows(row) + '</tbody></table>';
    return '<div class="ma-handle-page">' +
      '<div class="ma-handle-header">' +
        '<div class="ma-handle-title"><button class="btn btn-outline btn-sm" data-ma-action="back-list"><i class="bi bi-arrow-left"></i> 返回</button><span>' + (isDetail ? '元数据审核详情' : '元数据审核处理') + '</span><span class="ma-status ' + row.statusClass + '">' + row.status + '</span></div>' +
  ...
  return {
    html: '<div class="page-meta-audit">' +
      renderTabs('pending') +
    ...
    init: function () {
      var page = document.querySelector('.page-meta-audit');
      if (!page) return;
      bindListEvents(page);
